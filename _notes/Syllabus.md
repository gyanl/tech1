---
date: 24-07-2026
date modified: 04-08-2026
feed: show
title: "Syllabus"
---

See also: [[Lectures]] · [[Grading]] · [[AI Policy]]

**Course Title:** Tech 1 — Web & Mobile Ecosystem Fundamentals

**Course Objective:** Gain a foundational understanding of the core technologies, platforms, and concepts that underpin the modern web and mobile ecosystems, essential for product and design professionals.

**Format:** 13 lectures, 2 hours each, once a week.

### Key Areas Covered

| **S. No.** | **Contents**                                                                                          |
| ---------- | ----------------------------------------------------------------------------------------------------- |
| 1          | Internet & Web Basics — How the internet works (HTTP/HTTPS, DNS, IP addresses); client-server architecture; web browsers and their role |
| 2          | Front-End Technologies — HTML (structure), CSS (styling), JavaScript (interactivity); responsive web design |
| 3          | Back-End Technologies — Server-side languages, databases (SQL, NoSQL), APIs (REST, GraphQL); data flow |
| 4          | AI in the Stack — LLM APIs, AI features in products, AI-assisted development                           |
| 5          | Mobile Ecosystems — iOS vs. Android; native vs. hybrid vs. web apps; app stores and distribution       |
| 6          | Cloud Computing Basics — IaaS, PaaS, SaaS; hosting and deployment                                      |
| 7          | Development Methodologies — the software development lifecycle (SDLC); how software teams work         |

# Detailed Structure

#### Act I — Ship First (Weeks 1–3)
- [[Lecture 1]] — Hello, Internet: ship a page on day one
- [[Lecture 2]] — Anatomy of a Web Page: HTML, CSS, JavaScript
- [[Lecture 3]] — Talking to Servers: HTTP, DNS, IPs, client-server
- **Exercises:** [[Exercise - Ship a Page]] · [[Exercise - Web Page Anatomy]] · [[Exercise - Trace a Request]]

#### Act II — The Full Stack (Weeks 4–9)
- [[Lecture 4]] — Front-End in Practice: responsive design, design systems, Figma to code
- [[Lecture 5]] — Back-Ends and Databases: where data lives
- [[Lecture 6]] — APIs: how software talks to software
- [[Lecture 7]] — AI in the Stack: models, prompts, and AI features
- [[Lecture 8]] — Mobile Ecosystems: iOS, Android, and the in-betweens
- [[Lecture 9]] — Cloud Computing: someone else's computer
- **Exercises:** [[Exercise - Figma to Code]] · [[Exercise - Add a Database]] · [[Exercise - API Mashup]] · [[Exercise - Add an AI Feature]] · [[Exercise - Mobile Ecosystem Comparison]]

#### Act III — Build Something Real (Weeks 10–13)
- [[Lecture 10]] — How Software Gets Built: SDLC, agile, version control + **final project kickoff**
- [[Lecture 11]] — Studio 1: build sprint and crits
- [[Lecture 12]] — Studio 2: making it ship-ready (performance, security, privacy basics)
- [[Lecture 13]] — Demo Day
- **Exercise:** [[Exercise - SDLC Stage Mapping]] · **Project:** [[Project - Final Project]]
